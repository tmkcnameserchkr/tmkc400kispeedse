...
# ────────────────────────────────────────────────
# Replit → /home
# Railway → /data (agar volume mount kiya hai) warna fallback
# Render → /opt/render/project/src (adjust if needed)
# Local / VPS → home directory mein hidden folder
# Baaki platforms → current folder (lekin warning print)
...
def get_base_dir():
    ...
    elif os.getenv("RENDER_DISK_NAME"):  # Render ke liye
        return Path("/opt/render/project/src")  # ya apne hisaab se adjust karo
    else:
        # Normal local / VPS → hidden folder in home
...
# Debugging print (production mein comment out kar dena)
